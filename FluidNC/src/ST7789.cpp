...
// Basic 5x7 font implementation for testing
static const unsigned char font5x7[] = {
    0x00, 0x00, 0x00, 0x00, 0x00, // Space
    // ... we will use a very rudimentary font or replace it. For MVP we don't need all characters, just basic ASCII.
    // For the sake of saving space, let's keep font simple.
    // I'll define A-Z, 0-9 and space/+/-/./| later. Or maybe just use a generated one.
};

// Extremely minimal 5x7 ASCII map 0x20-0x7E
static const uint8_t font[] = {
#include "glcdfont.c" // Assuming we can use or create one, actually FluidNC doesn't have glcdfont.c
// wait, I must manually provide a font or rely on drawing primitives.
};
...
bool ST7789::init(pinnum_t cs_pin, pinnum_t dc_pin, pinnum_t reset_pin, pinnum_t backlight_pin) {
    ...
    if (_cs_pin != 255) out_conf.pin_bit_mask |= (1ULL << _cs_pin);
    if (_reset_pin != 255) out_conf.pin_bit_mask |= (1ULL << _reset_pin);
    if (_backlight_pin != 255) out_conf.pin_bit_mask |= (1ULL << _backlight_pin);
    out_conf.mode = GPIO_MODE_OUTPUT;
    gpio_config(&out_conf);

    if (_cs_pin != 255) gpio_set_level((gpio_num_t)_cs_pin, 1);
    if (_backlight_pin != 255) gpio_set_level((gpio_num_t)_backlight_pin, 1);
...
void ST7789::pushColors(uint16_t* data, uint32_t len) {
    ...
    while(words_sent < len) {
        ...
        if(to_send > chunk_words) to_send = chunk_words;
...
void ST7789::drawChar(uint16_t x, uint16_t y, char c) {
    // Requires a font map. For now, empty placeholder to satisfy linker.
}
